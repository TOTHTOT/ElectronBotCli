...
use sherpa_rs::tts::{VitsTts, VitsTtsConfig, TtsAudio};
...
fn test_tts(text: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Generated audio: {} samples, {} Hz, duration: {:.2}s",
        audio.samples.len(), audio.sample_rate, audio.duration);

    // Play audio
    let (_stream, stream_handle) = OutputStream::try_default()?;
    let sink = Sink::try_new(&stream_handle)?;

    let samples: Vec<f32> = audio.samples.iter()
...
fn test_asr() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Input config: {} Hz, {} channels", config.sample_rate(), config.channels());
    ...
    while start.elapsed().as_secs() < 10 {
        if let Ok(samples) = rx.recv_timeout(std::time::Duration::from_millis(100)) {
            ...
            while buffer.len() >= 1600 {
                ...
                let float_chunk: Vec<f32> = chunk.iter()
                    .map(|&s| s as f32 / i16::MAX as f32)
                    .collect();
