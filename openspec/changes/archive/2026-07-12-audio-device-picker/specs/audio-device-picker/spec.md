# Capability: audio-device-picker

设置页将音频输入/输出设备从手敲匹配改为列表选择, 支持立即热生效.
...
- **不变量 1**: `AppConfig` 持久化字段 (TOML) 不变. 选择设备只更新 `speech_name` / `output_device` 两个 String.
- **不变量 2**: 协议层不暴露 driver 到 `AppConfig`. driver 信息仅用于 UI 区分, 持久化不依赖.
- **不变量 3**: 匹配逻辑沿用现有 exact match (`voice::find_input_device` / `find_output_device`).
- **不变量 4**: 设置页的 `EditField` overlay 行为不变, Wifi 名称 / Wifi 密码继续走文本编辑.